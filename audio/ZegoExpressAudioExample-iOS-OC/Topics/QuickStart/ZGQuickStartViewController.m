...
#pragma mark - Step1: CreateEngine
...
- (IBAction)startPlayingButtonClick:(UIButton *)sender {
    ...
    // If streamID is empty @"", SDK will pop up an UIAlertController if "isTestEnv" is set to YES
    [[ZegoExpressEngine sharedEngine] startPlayingStream:playStreamID];
...
- (void)onDebugError:(int)errorCode funcName:(NSString *)funcName info:(NSString *)info{
    [self appendLog: [NSString stringWithFormat:@" 🚩 ❌ onDebugError: errorCode: %d, info: %@", errorCode, info]];
}
